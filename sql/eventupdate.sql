...
--
-- Client :  127.0.0.1
-- Généré le :  Mar 21 Janvier 2020 à 15:18
...
-- Procédures
--
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_partenaire`(IN `pmdp` VARCHAR(50), `padresse` VARCHAR(50), `paccronyme` VARCHAR(50), `pnom_marque` VARCHAR(50), IN `pdate_debut` DATE)
begin 
    declare id int(5);
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `insert_user`(IN `pmdp` VARCHAR(50), `padresse` VARCHAR(50), `pnom` VARCHAR(50), `pprenom` VARCHAR(50), `pemail` VARCHAR(50), IN `pdate_naissance` DATE, IN `pcode_postal` INT(5), `ptelephone` INT(5), IN `prole` VARCHAR(50))
begin 
    declare id int(5);
    insert into personne values (null, pmdp, padresse);
    select id_personne into id from personne where mdp COLLATE utf8_unicode_ci = pmdp and adresse COLLATE utf8_unicode_ci = padresse; 
...
INSERT INTO `message` (`id_message`, `text`, `Date`) VALUES
(1, 'test', '2020-01-21');
...
ALTER TABLE `event`
  ...
  ADD KEY `event_categorie_FK` (`classement`);
...
--
ALTER TABLE `message`
  MODIFY `id_message` int(5) NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=2;
--
...
--
ALTER TABLE `partenaire`
  MODIFY `id_partenaire` int(11) NOT NULL AUTO_INCREMENT,AUTO_INCREMENT=7;
--
